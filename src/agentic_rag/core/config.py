...
class Settings:
    ...
    chat_api_key: str | None = os.getenv("API_KEY")
    chat_base_url: str = os.getenv("BASE_URL", "https://api.deepseek.com")
    chat_model: str = os.getenv("MODEL", "deepseek-chat")
    chat_timeout_seconds: float = float(os.getenv("TIMEOUT_SECONDS", "300"))
    chat_max_retries: int = int(os.getenv("MAX_RETRIES", "4"))
    chat_retry_base_delay_seconds: float = float(os.getenv("RETRY_BASE_DELAY_SECONDS", "1.0"))
    chat_retry_max_delay_seconds: float = float(os.getenv("RETRY_MAX_DELAY_SECONDS", "8.0"))
